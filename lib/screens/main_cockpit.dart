...
class _MainCockpitState extends State<MainCockpit> with WidgetsBindingObserver {
  ...
  // CLAUDE FIX 6: Proje index'i için de sınır koruması eklendi.
  ...
  // CLAUDE FIX 2: GPS Simülasyonu artık sahayı yanıltmayacak şekilde etiketlendi
  Future<void> _fetchGPSData() async {
    showDialog(
      context: context, barrierDismissible: false,
      builder: (c) => const AlertDialog(
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          CircularProgressIndicator(color: Colors.orangeAccent), SizedBox(height: 15),
          Text('⚠️ SİMÜLASYON MODU\nUydu verisi taklit ediliyor...', textAlign: TextAlign.center, style: TextStyle(fontSize: 12)),
        ]),
      ),
    );
    await Future.delayed(const Duration(seconds: 2));
    Navigator.pop(context); 
    setState(() { p.altitudeMeters = 1250; }); 
    _persistProjects();
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(backgroundColor: Colors.orange, content: Text('⚠️ DEMO: Rakım manuel olarak 1250m atandı. Gerçek sensör verisi değildir.')));
    }
  }

  // CLAUDE FIX 2: OCR Simülasyonu etiketlendi
  Future<void> _scanOCR() async {
    showDialog(
      context: context, barrierDismissible: false,
      builder: (c) => const AlertDialog(
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          Icon(Icons.document_scanner, size: 40, color: Colors.orangeAccent), SizedBox(height: 15),
          LinearProgressIndicator(color: Colors.orangeAccent), SizedBox(height: 15),
          Text('⚠️ SİMÜLASYON MODU\nÖrnek röle etiketi okunuyor...', textAlign: TextAlign.center, style: TextStyle(fontSize: 12)),
        ]),
      ),
    );
    await Future.delayed(const Duration(seconds: 2));
    Navigator.pop(context);
    setState(() { p.up50PickupA = 1250; p.upTms = 0.15; }); 
    _persistProjects();
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(backgroundColor: Colors.orange, content: Text('⚠️ DEMO: Pick-up akımı simülasyon amacıyla 1250A yapıldı.')));
    }
  }
  ...
  Future<void> _loadPreferencesAndProjects() async { 
    try { 
      ...
      final savedLang = prefs.getString('powerfield_lang_v68'); 
      if (savedLang != null && mounted) setState(() => currentLang = savedLang); 
    } catch (_) {} 
  }
  
  void _showModeChooser() { 
    // CLAUDE FIX 4: Bayrak mantığı düzeltildi, bir daha sorulmaması için true yapıyoruz.
  ...
  Widget _cockpit() => ListView(padding: const EdgeInsets.all(12), children: [
    // CLAUDE FIX 6: Deprecated withOpacity yerini withAlpha aldı (veya doğrudan alfa kanallı Color kullanımı)
    Container(
      padding: const EdgeInsets.all(8), margin: const EdgeInsets.only(bottom: 10),
      decoration: BoxDecoration(color: const Color(0x1A448AFF), borderRadius: BorderRadius.circular(5), border: Border.all(color: const Color(0x4D448AFF))),
      child: Row(children: [
    ...
    _sliderWithInfo(t('volt'), p.voltageKv, .4, 380, (v) => p.voltageKv = v, 'Tesis barasının nominal çalışma gerilimi (Un).'),
    _sliderWithInfo(t('trafo'), p.trafoMva, .1, 250, (v) => p.trafoMva = v, 'Güç trafosunun etiketindeki anma gücü (Sr).'),
    _sliderWithInfo('Kısa Devre Empedansı (%uk)', p.ukPercent, 3, 18, (v) => p.ukPercent = v, 'Trafonun empedans gerilimi. Bu değer ne kadar düşükse, trafonun basacağı kısa devre tepe akımı o kadar şiddetli olur.'),
    _sliderWithInfo('Şebeke Kısa Devre Gücü (Ssc MVA)', p.gridSscMva, 200, 5000, (v) => p.gridSscMva = v, 'TEİAŞ/TEDAŞ bağlantı noktasındaki (PCC) kısa devre görünür gücü.'),
  ]);
  ...
  Widget _relayTab() { 
    ...
    return ListView(padding: const EdgeInsets.all(12), children: [
      _card('50/51 AŞIRI AKIM KOORDİNASYONU', Column(children: [
        ...
        _sliderWithInfo('I> Pick-up (Amper)', p.up50PickupA, 10, 5000, (v) => p.up50PickupA = v, 'Faz aşırı akım başlama (kalkış) eşiği.'), 
        _sliderWithInfo('TMS (Zaman Çarpanı)', p.upTms, 0.02, 1.2, (v) => p.upTms = v, 'Ters zamanlı eğride (IDMT) dikey eksende kaydırmayı sağlar.'), 
  ...
  Widget _cableTab() { 
    ...
    return ListView(padding: const EdgeInsets.all(12), children: [
      _card('KABLO KESİT TAYİNİ (IEC 60364)', Column(children: [
        ...
        _sliderWithInfo('Uzunluk (Metre)', p.cableLengthM, 10, 1000, (v) => p.cableLengthM = v, 'Fider ile yük arasındaki kablo metrajı.'), 
  ...
  Widget _analysisTab() {
    ...
    return ListView(padding: const EdgeInsets.all(12), children: [
      _card('YÜK AKIŞI & GERİLİM DÜŞÜMÜ', Column(children: [
        ...
        _sliderWithInfo('Güç Faktörü (cos φ)', p.loadFlowPf, 0.5, 1.0, (v) => p.loadFlowPf = v, 'Tesisin güç faktörü.'), 
  ...
  Widget _satTab() => ListView(padding: const EdgeInsets.all(12), children: [
    _card('KONTAK GEÇİŞ DİRENCİ (MİKRO-OHM)', Column(children: [
      ...
      _sliderWithInfo('Faz R (L1) - µΩ', p.resR, 5, 150, (v) => p.resR = v, 'L1 fazının ana kontak kapalı konumdaki geçiş direnci.'), 
  ...
  // CLAUDE FIX 1: Parantez hatası tamamen düzeltildi.
  Widget _resultRow(String label, String value) => Padding(
    ...
    child: Row(
      ...
      children: [
        ...
        Text(value, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.greenAccent))
      ]
    )
  );
  ...
  Widget _sld() => ListView(padding: const EdgeInsets.all(12), children: [
    ...
    ...p.cells.map((c) => Card(child: ListTile(dense: true, title: Text(c.name, style: const TextStyle(fontSize: 11)), subtitle: Text('${c.currentVendor} | ${c.currentBreaker} | CT ${c.ctRatio}', style: const TextStyle(fontSize: 9)), trailing: Switch(value: c.cbClosed, onChanged: (v) { setState(() => c.cbClosed = v); _persistProjects(); }))))
  ]);

  // CLAUDE FIX 3: onChanged içine _persistProjects eklendi.
  Widget _sliderWithInfo(String title, double value, double minV, double maxV, ValueChanged<double> onChanged, String infoText) => Card(
    ...
    child: Padding(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 8), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Slider(value: value.clamp(minV, maxV).toDouble(), min: minV, max: maxV, onChanged: (v) { setState(() => onChanged(v)); _persistProjects(); })
    ]))
  );
