...
class ElectricalEngine {
  ...
  static double calcNominalCurrentA(double mva, double voltageKv) =>
      voltageKv <= 0 ? 0.0 : (mva * 1000) / (sqrt(3) * voltageKv);
  ...
  static double calcRocofHzSec({
    ...
    if (sg <= 0 || inertiaH_Sec <= 0) return 0.0;
  ...
  static double calcResonanceOrder(double sscMva, double qcKvar) {
    if (sscMva <= 0 || qcKvar <= 0) return 0.0;
  ...
  static double calcPrimaryPickupAmps({
    ...
    required double secondarySettingA,
      ...
      ctSecondaryA <= 0 ? 0.0 : secondarySettingA * ctPrimaryA / ctSecondaryA;
  ...
  static double calcTripTime({
    ...
    if (standard == ProtectionStandard.iec60255) {
      ...
      switch (curve) {
        case TripCurve.veryInverse: k = 13.5; alpha = 1.0;
        case TripCurve.extremelyInverse: k = 80.0; alpha = 2.0;
        case TripCurve.longTimeInverse: k = 120.0; alpha = 1.0;
    ...
    switch (curve) {
      case TripCurve.veryInverse: a = 19.61; b = .491; p = 2.0;
      case TripCurve.extremelyInverse: a = 28.2; b = .1217; p = 2.0;
      case TripCurve.longTimeInverse: a = 120.0; b = 0.0; p = 1.0;
      case TripCurve.standardInverse: break;
    }
    final d = pow(m, p) - 1;
  ...
  static Map<String, dynamic> evaluateCableSizing({
    ...
    final sMin = ikKa * 1000 * sqrt(max(faultTimeSec, 0.0)) / k;
    ...
    final sinPhi = sqrt(max(0.0, 1 - powerFactor * powerFactor));
  ...
  static Map<String, double> calcLoadFlow({
    ...
    if (voltageKv <= 0 || activePowerMw < 0 || lineLengthKm < 0) return {'currentA': 0.0, 'qMvar': 0.0, 'sendingKv': 0.0, 'receivingKv': 0.0, 'dropPercent': 0.0, 'lossMw': 0.0};
    final qMvar = activePowerMw * sqrt(max(0.0, 1 / (pf * pf) - 1));
  ...
  static Map<String, double> calcHarmonics({
    ...
    for (int i = 0; i < n; i++) {
      ...
      final impedanceOhm = systemVoltageKv <= 0 || shortCircuitMva <= 0 ? 0.0 : (systemVoltageKv * systemVoltageKv / shortCircuitMva) * order;
      final vhPercent = systemVoltageKv <= 0 ? 0.0 : 100 * ih * impedanceOhm / (systemVoltageKv * 1000 / sqrt(3));
      thdv2 += vhPercent * vhPercent;
    }
    final thdi = fundamentalCurrentA <= 0 ? 0.0 : 100 * sqrt(sumI2) / fundamentalCurrentA;
